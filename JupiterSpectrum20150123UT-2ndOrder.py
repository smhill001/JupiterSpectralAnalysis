...
CLR = scipy.fromfile(file="Data/20150123UT/JupiterSpectrum-20150122LT-CLR-sum50m00s-Rotated-Cropped-WVCal-2ndOrder.dat", dtype=float, count=-1, sep='\t')    
NIR = scipy.fromfile(file="Data/20150123UT/JupiterSpectrum-20150122LT-742-sum3h20m-Rotated-Cropped-WVCal-2ndOrder.dat", dtype=float, count=-1, sep='\t')    
NormResponsewithWV= scipy.fromfile(file="PolluxResponse20150123UT-4thOrder.txt", dtype=float, count=-1, sep=" ")
...
EWFN="JupiterEW20150123UT-2ndOrder.txt"
...
BoxcarSmoothIndices=np.where(Ref[:,0] >= 9000.)
NResponRefSmooth=deepcopy(NResponRef)
NResponRefSmooth[BoxcarSmoothIndices]=scipy.convolve(NResponRef[BoxcarSmoothIndices],np.ones((9,))/9)[(8):]
ToA=deepcopy(MASTER)
...
tempwave=Ref[:,0]
tempsignal=Ref[:,1]
TmpRefWV=Ref[:,0]+5.*(N/2.-0.5)
TmpRefSig=scipy.convolve(tempsignal,np.ones((N,))/N)[(N-1):]

RefInterp=interpolate.interp1d(TmpRefWV,TmpRefSig,kind='linear', copy=True,
                         bounds_error=False, fill_value=0.0)  
RefSig=RefInterp(Ref[:,0])
Ref[:,1]=RefSig
...
np.savetxt("JupiterAlbedo20150123UT-2ndOrder.txt",NormAlbedowithWV,delimiter=" ",fmt="%10.3F %10.7F")
#Begin plotting 
...
pl.title("Jupiter Spectrum 20150123UT-2ndOrder",fontsize=9)
...
pylab.savefig('JupiterSpectrum20150123UT-2ndOrder.png',dpi=300)


TempMaster=MASTER
TempMaster[:,0]=MASTER[:,0]/10.
TempMaster[:,1]=MASTER[:,1]/(ExposureCLR*Aperture*NativeDispersionNM)
np.savetxt("JupiterSpectrum20150123UT-2ndOrder.txt",MASTER,delimiter=" ",fmt="%10.3F %10.7F")
np.savetxt("JupiterAlbedo20150123UT-2ndOrder.txt",NormAlbedowithWV,delimiter=" ",fmt="%10.3F %10.7F")